...
unsigned int Crawler_v2_parse_seconds(char *str)
{
	char *end;
...
int main(int argc, char *argv[])
{
	...
	char *host = getenv("Crawler_v2_HOST");
	char *pass = getenv("Crawler_v2_PASS");
	char *port = getenv("Crawler_v2_PORT");
	
	if (!port) port = "25575";
	if (!host) host = "localhost";

	// disable output buffering (https://github.com/Tiiffi/Crawler_v2/pull/39)
	...
	while ((opt = getopt(argc, argv, "vrtcshw:H:p:P:")) != -1)
	{
		switch (opt) {
			...
			case 'w':
				global_wait_seconds = Crawler_v2_parse_seconds(optarg);
			break;

			case 'v':
				puts(VER_STR" - https://github.com/Tiiffi/Crawler_v2");
				puts("Bug reports:\n\ttiiffi+Crawler_v2 at gmail\n\thttps://github.com/Tiiffi/Crawler_v2/issues/");
				exit(EXIT_SUCCESS);

			case 'h': usage(); break;
			case '?':
			default:
				puts("Try 'Crawler_v2 -h' or 'man Crawler_v2' for help.");
				exit(EXIT_FAILURE);
		}
	}

	if (pass == NULL) {
		puts("You must give password (-p password).\nTry 'Crawler_v2 -h' or 'man Crawler_v2' for help.");
		return 0;
	}
...
void usage(void)
{
	...
	puts (
		"- Crawler_v2 will start in terminal mode if no commands are given\n"
